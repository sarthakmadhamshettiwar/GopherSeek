	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < concurrentQueries; j++ {
			...
			go func() {
				...
				rows, err := conn.Query(ctx, "SELECT id, text FROM docs;")
				if err != nil {
...
// This is the correct way to use a connection pool.
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < concurrentQueries; j++ {
			...
			go func() {
				...
				ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
				defer cancel()
				rows, err := pool.Query(ctx, "SELECT id, text FROM docs;")
				if err != nil {
...
// BenchmarkDB_PoolPerCall_100Concurrent mirrors the current getCorpusFromDBPool() implementation —
